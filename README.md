...
synthetic\_drilling\_data\_generator.py

&#x20;       │ (CSV: data/raw/drilling\_telemetry.csv)

&#x20;       ▼

preprocess → feature\_engineering → sequence\_gen
...
batch\_predict.py → SQLite predictions → API → Dashboard
...
python backend/data/synthetic\_drilling\_data\_generator.py



\# 4. Train models (logs to MLflow)

python backend/training/train\_pipeline.py



\# 5. Run batch predictions + seed SQLite

python backend/training/batch\_predict.py
...
DATABASE\_URL=sqlite:///./data/drilling.db

MLFLOW\_TRACKING\_URI=./mlflow/mlruns

MODEL\_DIR=backend/models/artifacts
