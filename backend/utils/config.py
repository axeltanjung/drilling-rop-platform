...
# Project root = two levels up from this file (backendutilsconfig.py - root)
...
class Settings
    ...
    def __init__(self) - None
        ...
        self.database_url str = os.getenv(
            DATABASE_URL, fsqlite{ROOT_DIR  'data'  'drilling.db'}
        )

        # Paths
        self.data_dir Path = Path(os.getenv(DATA_DIR, ROOT_DIR  data))
        self.raw_data_path Path = Path(
            os.getenv(RAW_DATA_PATH, self.data_dir  raw  drilling_telemetry.csv)
        )
        self.processed_dir Path = self.data_dir  processed
        self.model_dir Path = Path(os.getenv(MODEL_DIR, ROOT_DIR  backend  models  artifacts))
